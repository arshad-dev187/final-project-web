...
const tabs = ['overview', 'orders', 'categories', 'messages'];
const orderStatusLabel = status => ({ pending: 'Pending', accepted: 'Accepted', in_progress: 'In Progress', completed: 'Completed', rejected: 'Rejected' }[status] || (status ? String(status).replace(/_/g, ' ') : ''));
const statCards = [
...
function RadarChart({ items }) {
  ...
  const max = Math.max(...items.map(i => Number(i.orders || 0)), 1);
  const angle = (2 * Math.PI) / items.length;
  const point = (i, r) => ({ x: cx + r * Math.cos(-Math.PI / 2 + i * angle), y: cy + r * Math.sin(-Math.PI / 2 + i * angle) });
  const polygon = items.map((item, i) => { const p = point(i, (Number(item.orders || 0) / max) * radius); return `${i ? 'L' : 'M'}${p.x},${p.y}`; }).join(' ');
  return <svg viewBox={`0 0 ${size} ${size}`} className="radar-chart" role="img" aria-label="Items performance radar chart">
    ...
    {items.map((item, i) => { const p = point(i, (Number(item.orders || 0) / max) * radius); return <circle key={i} cx={p.x} cy={p.y} r="3" fill="#163d32" />; })}
...
export default function AdminDashboardPage() {
  ...
  const refresh = async () => { try { const [summary, categoryRows, messageRows, orderRows] = await Promise.all([api('/dashboard/stats'), api('/categories'), api('/messages'), api('/orders')]); setStats(summary); setCategories(categoryRows); setMessages(messageRows); setOrders(orderRows); } catch (err) { setError(err.message); } };
  const loadAnalytics = async () => { const params = new URLSearchParams({ range: salesRange }); if (salesRange === 'custom') { if (/^\d{4}-\d{2}-\d{2}$/.test(customFrom)) params.set('start', customFrom); if (/^\d{4}-\d{2}-\d{2}$/.test(customTo)) params.set('end', customTo); } try { setAnalytics(await api(`/dashboard/analytics?${params.toString()}`)); } catch (err) { setError(err.message); } };
  useEffect(() => { refresh(); }, []);
  useEffect(() => { loadAnalytics(); }, [salesRange, customFrom, customTo]);
  ...
  const updateOrderStatus = async (orderId, status) => { try { await api(`/orders/${orderId}/status`, jsonOptions({ status }, 'PUT')); setNotice('Order status updated successfully.'); await refresh(); } catch (err) { setError(err.message); } };
  const salesData = (analytics?.sales || []).map(d => ({ day: d.label, total: Number(d.total ?? 0) }));
